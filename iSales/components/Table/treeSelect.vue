    ...
    <div class="the_tree_select">
      <el-row>
        <el-col :span="24" style="position: relative;padding: 0;">
          <el-input v-model="saleOrgCode" @clear="clearOptions" @blur="getOrg" clearable :disabled="disabled"/><el-button icon="el-icon-search" class="the_tree_btn" @click="openDialog" :disabled="disabled"/>
        </el-col>
      </el-row>
...
export default{
  name: 'treeSelect',
    ...
    getOrg: debounce(function(val){
      if(!this.saleOrgCode) return;
      if (this.allowInput ) {
        //允许录入的情况下直接赋值-不要再做请求数据
        this.$emit("close-tree", {orgCode:this.saleOrgCode}, this.scopeData);
        this.saleOrgCode = this.saleOrgCode;
        return;
      }
      /*if(!this.subOrgCode) {
        this.$mideaMessage({
